...
public class LunarExecutorTest {
    ...
    public void tearDown() throws Exception {
        System.out.println("Tearning down environment...");
    ...
    @Test(timeOut = 1, groups = "group2", dependsOnMethods = "testSolarToInt")
...
//        Thread.sleep(1);
    ...
    public void testLunarToSolar() throws Exception {
        assertEquals(LunarExecutor.LunarToSolar(new Lunar(2015, 1, 1, false)), new Solar(2015, 2, 19));
        assertEquals(LunarExecutor.LunarToSolar(new Lunar(2015, 1, 29, false)), new Solar(2015, 3, 19));
        assertEquals(LunarExecutor.LunarToSolar(new Lunar(2015, 11, 21, false)), new Solar(2015, 12, 31));
        assertEquals(LunarExecutor.LunarToSolar(new Lunar(2015, 12, 29, false)), new Solar(2016, 2, 7));
        assertEquals(LunarExecutor.LunarToSolar(new Lunar(2016, 1, 1, false)), new Solar(2016, 2, 8));

    }

    @Test(expectedExceptions = {ArrayIndexOutOfBoundsException.class})
    public void testLunarToSolarThrowOutOfBoundException() throws Exception {
        assertEquals(LunarExecutor.LunarToSolar(new Lunar(9999, 1, 1, false)), new Solar(2016, 2, 8));
    }

    @Test(dataProvider = "s2l")
    @Parameters(value = "solar2lunar")
    public void testSolarToLunar(Solar[] s,Lunar[] l) throws Exception {
        for (int i = 0; i < s.length; i++) {
            assertEquals(LunarExecutor.SolarToLunar(s[i]),l[i]);
        }
    }
    ...
    @Test(dependsOnMethods = "thisTestWillFail",enabled = false)
    public void thisTestWontRun() {
        assertEquals(1, 1);
    }

    @DataProvider(name = "s2l")
    public Object[][] parametersSolor2Lunar() {
        Solar[] s = {new Solar(2015, 1, 1),         new Solar(1901, 2, 19),         new Solar(2012, 1, 23)};
        Lunar[] l = {new Lunar(2014, 11, 11, false), new Lunar(1901, 1, 1, false), new Lunar(2012, 1, 1, true)};
